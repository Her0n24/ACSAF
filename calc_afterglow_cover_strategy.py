...
def get_or_save_latlon(location_name, filename="location_latlon.txt"):
    ...
    if os.path.exists(filename):
        with open(filename, "r", encoding="utf-8") as f:
            ...
            name, lat, lon = line.split(",", 2)  # Only split on the first two commas
...
# Usage:
lat, lon = get_or_save_latlon("Reading, England")
lat, lon = round(lat, 1), round(lon, 1)  # Ensure lat/lon are rounded to 1 decimal place

#logging.info(location): print the name of the location

city = LocationInfo("Reading", "England", "Europe/London", lat, lon)  # Hong Kong
...
logging.info(city.name, lat, lon)
...
get_cams_aod(today, run, city.name, today_str, input_path)
...
ds_18 = xr.open_dataset(f'{input_path}/{today_str}{run}0000-18h-oper-fc.grib2', engine = 'cfgrib')
...
distance_below_threshold_18, key_18, avg_first_three_18, avg_path_18, cloud_present_18, cloud_base_lvl_18  = get_cloud_extent(cloud_vars_18, lon, lat, sunset_azimuth, cloud_base_lvl_18, '18')
distance_below_threshold_42, key_42, avg_first_three_42, avg_path_42, cloud_present_42, cloud_base_lvl_42  = get_cloud_extent(cloud_vars_42, lon, lat, sunset_azimuth, cloud_base_lvl_42, '42')
...
logging.info(cloud_base_lvl_18)
logging.info(cloud_base_lvl_42)
...
def create_dashboard(index_today, index_tomorrow, city, latitude, longitude,
    ...
    info_text = (
        ...
        f"Cloud Height : {int(round(cloud_base_lvl_18, -2))} m\n"
    ...
    info_text = (
        ...
        f"Cloud Height : {int(round(cloud_base_lvl_42, -2))} m\n"
